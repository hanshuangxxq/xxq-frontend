...
const availableColumns = computed<DataTableColumns<InternshipResponse>>(() => [
  ...
  },
  {
    ...
    render: (row) => {
      ...
      if (applied) {
        return h(NButton, { size: 'small', type: 'primary', secondary: true, disabled: true }, () =>
          t('practice.internship.applied'),
        )
      }
      return h(NButton, { size: 'small', type: 'primary', onClick: () => startApply(row) }, () =>
        t('practice.internship.apply'),
      )
    },
  },
])
...
const myAppColumns = computed<DataTableColumns<InternshipApplicationResponse>>(() => [
  ...
  {
    title: t('practice.common.reviewTime'),
    key: 'reviewTime',
    width: 150,
    render: (r) => formatDateTime(r.reviewTime),
  },
  {
...
const internshipOptions = computed(() =>
  approvedInternships.value.map((a) => ({ label: a.internshipTitle, value: a.internshipId })),
)

const showReportForm = ref(false)
const reportFormMode = ref<'create' | 'edit'>('create')
const reportForm = ref<{ internshipId: number | null; title: string; summary: string }>({
  internshipId: null,
  title: '',
  summary: '',
})
...
const reportColumns = computed<DataTableColumns<InternshipReportResponse>>(() => [
  ...
  {
    title: t('practice.common.feedback'),
    key: 'feedback',
    width: 160,
    ellipsis: { tooltip: true },
    render: (r) => r.feedback || '-',
    ...
    render: (row) =>
      h(NSpace, { size: 8 }, () => [
        row.status === '已提交'
          ? h(
              NButton,
              { size: 'small', type: 'primary', onClick: () => startResubmitReport(row) },
              () => t('practice.internship.resubmitReport'),
            )
          : null,
...
const availTrainColumns = computed<DataTableColumns<TrainingResponse>>(() => [
  ...
  },
  {
    ...
    width: 100,
    render: (row) =>
      h(NButton, { size: 'small', type: 'primary', onClick: () => handleEnroll(row) }, () =>
        t('practice.internship.enroll'),
      ),
  },
])
  ...
  },
  {
    ...
    render: (row) =>
      row.status === '已报名'
        ? h(
            NPopconfirm,
            { onPositiveClick: () => handleCancelEnroll(row.id) },
            {
              default: () => t('practice.internship.cancelEnrollConfirm'),
              trigger: () =>
                h(NButton, { size: 'small', type: 'warning' }, () =>
                  t('practice.internship.cancelEnroll'),
                ),
            },
          )
        : '-',
        ...
        <NTabPane name="myReports" :tab="$t('practice.internship.tabMyReports')">
          <NCard :title="$t('practice.internship.tabMyReports')">
            <template #header-extra>
              <NButton
                v-if="approvedInternships.length > 0"
                type="primary"
      ...
      >
        <NForm :model="reportForm" label-placement="top">
          ...
          <NFormItem :label="$t('practice.common.summary')">
            <NInput
              v-model:value="reportForm.summary"
              type="textarea"
        ...
        <template #footer>
          <NSpace justify="end">
            ...
            <NButton type="primary" :loading="savingReport" @click="handleSubmitReport">{{
