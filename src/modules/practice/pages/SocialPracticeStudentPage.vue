...
/**
 * 学生社会实践页(仅学生)。三个 Tab:可申报项目(个人/团队,团队成员从学生列表多选)、
...
const availableColumns = computed<DataTableColumns<SocialPracticeResponse>>(() => [
  ...
  },
  {
    ...
    width: 100,
    render: (row) =>
      h(NButton, { size: 'small', type: 'primary', onClick: () => startApply(row) }, () =>
        t('practice.socialPractice.apply'),
      ),
  },
])
            ...
            { onPositiveClick: () => handleRevoke(row.id) },
            {
              ...
              trigger: () =>
                h(NButton, { size: 'small', type: 'warning' }, () =>
                  t('practice.socialPractice.revoke'),
                ),
            },
          )
        : '-',
...
const practiceOptions = computed(() =>
  approvedPractices.value.map((a) => ({ label: a.practiceTitle, value: a.practiceId })),
)

const showReportForm = ref(false)
const reportFormMode = ref<'create' | 'edit'>('create')
const reportForm = ref<{ practiceId: number | null; title: string; summary: string }>({
  practiceId: null,
  title: '',
  summary: '',
})
      ...
      h(NSpace, { size: 8 }, () => [
        row.status === '已提交'
          ? h(
              ...
              () => t('practice.socialPractice.resubmitReport'),
            )
          : null,
  ...
  <div class="practice-page">
    ...
    <template v-else>
        ...
        <NTabPane name="myReports" :tab="$t('practice.socialPractice.tabMyReports')">
          <NCard :title="$t('practice.socialPractice.tabMyReports')">
            <template #header-extra>
              <NButton
                v-if="approvedPractices.length > 0"
                type="primary"
      ...
      <NModal
        ...
        preset="card"
        :title="$t('practice.socialPractice.apply')"
