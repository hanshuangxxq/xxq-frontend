...
import { auditStatusTagType, formatDateTime, parseMembers, joinMembers } from '../utils'
import type { Student } from '@/modules/student-management/types'
import type { CompetitionResponse, RegistrationResponse, CompetitionResultResponse } from '../types'
  ...
  },
  {
    ...
    render: (row) =>
      h(NButton, { size: 'small', type: 'primary', onClick: () => startRegister(row) }, () =>
        t('practice.competition.register'),
      ),
  },
])
...
const myResultColumns = computed<DataTableColumns<RegistrationResponse>>(() => [
  ...
  },
  {
    ...
    width: 120,
    render: (row) =>
      h(NButton, { size: 'small', onClick: () => openMyResult(row) }, () =>
        t('practice.competition.viewResult'),
      ),
  },
])
  ...
  <div class="practice-page">
    ...
    <template v-else>
      ...
      <NModal
        ...
        preset="card"
        :title="$t('practice.competition.register')"
      ...
      >
        ...
        <template #footer>
          <NSpace justify="end">
            ...
            <NButton type="primary" :loading="savingRegister" @click="handleRegister">{{
