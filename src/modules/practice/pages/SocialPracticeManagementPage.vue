...
const statusDropdown = computed(() => [
  { label: t('practice.socialPractice.statusDraft'), key: 'DRAFT' },
  { label: t('practice.socialPractice.statusOpen'), key: 'OPEN' },
  { label: t('practice.socialPractice.statusClosed'), key: 'CLOSED' },
])
const reportStatusOptions = computed(() => [
  { label: t('practice.socialPractice.reportStatusSubmitted'), value: 'SUBMITTED' as ReportStatusCode },
  { label: t('practice.socialPractice.reportStatusReviewed'), value: 'REVIEWED' as ReportStatusCode },
])
...
const reviewAppForm = ref<{ approved: boolean; reviewComment: string }>({ approved: true, reviewComment: '' })
...
const practiceColumns = computed<DataTableColumns<SocialPracticeResponse>>(() => [
  ...
  },
  {
    ...
    render: (row) =>
      h(NSpace, { size: 8 }, () => [
        h(NButton, { size: 'small', onClick: () => startEditPr(row) }, () => t('practice.common.edit')),
        h(NDropdown, { options: statusDropdown.value, onSelect: (key: string) => handlePrStatusChange(row, key) }, () =>
          h(NButton, { size: 'small' }, () => t('practice.common.status')),
        ),
        h(NButton, { size: 'small', onClick: () => openApps(row) }, () => t('practice.socialPractice.viewApplications')),
        h(NPopconfirm, { onPositiveClick: () => handleDeletePractice(row.id) }, {
...
const appColumns = computed<DataTableColumns<SocialPracticeApplicationResponse>>(() => [
  ...
  { title: t('practice.socialPractice.applyReason'), key: 'applyReason', minWidth: 140, ellipsis: { tooltip: true }, render: (r) => r.applyReason || '-' },
  { title: t('practice.common.reviewComment'), key: 'reviewComment', width: 150, ellipsis: { tooltip: true }, render: (r) => r.reviewComment || '-' },
  {
    ...
    render: (row) =>
      row.status === '待审核'
        ? h(NButton, { size: 'small', type: 'primary', onClick: () => startReviewApp(row) }, () => t('practice.socialPractice.reviewApply'))
        : '-',
  },
])
...
const reportColumns = computed<DataTableColumns<SocialPracticeReportResponse>>(() => [
  ...
  { title: t('practice.socialPractice.reportScore'), key: 'score', width: 80, align: 'center', render: (r) => r.score ?? '-' },
  {
    ...
    render: (row) =>
      h(NSpace, { size: 8 }, () => [
        h(NButton, { size: 'small', type: 'primary', onClick: () => startReviewReport(row) }, () => t('practice.socialPractice.reviewReport')),
...
<template>
  <div class="practice-page">
        ...
        <NCard :title="$t('practice.socialPractice.tabPractices')">
          <template #header-extra>
            <NSpace align="center">
              ...
              />
              <NButton type="primary" @click="loadPractices">{{ $t('practice.common.query') }}</NButton>
              <NButton @click="handlePrFilterChange">{{ $t('practice.common.reset') }}</NButton>
              <NButton type="primary" @click="startCreatePr">{{ $t('practice.socialPractice.addPractice') }}</NButton>
            </NSpace>
          </template>
    ...
    <NModal
      ...
      preset="card"
      :title="prFormMode === 'create' ? $t('practice.socialPractice.addPractice') : $t('practice.socialPractice.editPractice')"
      ...
      <NForm :model="prForm" label-placement="top">
        ...
        <NSpace :size="12" wrap>
          <NFormItem :label="$t('practice.common.startTime')" style="width: 240px">
            <NDatePicker v-model:value="prForm.startTs" type="datetime" clearable style="width: 100%" />
          </NFormItem>
          <NFormItem :label="$t('practice.common.endTime')" style="width: 240px">
            <NDatePicker v-model:value="prForm.endTs" type="datetime" clearable style="width: 100%" />
          </NFormItem>
        </NSpace>
        <NFormItem :label="$t('practice.common.description')">
          <NInput v-model:value="prForm.description" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" />
        </NFormItem>
      </NForm>
      <template #footer>
        <NSpace justify="end">
          <NButton @click="showPrForm = false">{{ $t('practice.common.cancel') }}</NButton>
          <NButton type="primary" :loading="savingPr" @click="handleSavePr">{{ $t('practice.common.save') }}</NButton>
        </NSpace>
      </template>
    </NModal>
    ...
    <!-- 申报审核 -->
    <NModal v-model:show="showReviewApp" preset="card" :title="$t('practice.socialPractice.reviewApply')" class="practice-form-modal">
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton type="primary" :loading="savingReviewApp" @click="handleSaveReviewApp">{{ $t('practice.common.confirm') }}</NButton>
        </NSpace>
      </template>
    </NModal>

    <!-- 报告评审 -->
    <NModal v-model:show="showReviewReport" preset="card" :title="$t('practice.socialPractice.reviewReport')" class="practice-form-modal">
