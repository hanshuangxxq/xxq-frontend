...
const baseColumns: DataTableColumns<Course> = [
  { title: t('course-management.courseName'), key: 'courseName', width: 160, ellipsis: { tooltip: true } },
  ...
  <div class="cm-page">
    <NSpace vertical :size="16">
      <NCard :title="$t('course-management.title')">
        ...
        <NSpin :show="loading">
          <NEmpty v-if="!loading && data.length === 0" :description="$t('course-management.empty')" />
          <NDataTable
            v-else
            :columns="columns"
            :data="data"
    ...
    <NModal
      ...
      preset="card"
      :title="formMode === 'create' ? $t('course-management.createTitle') : $t('course-management.editTitle')"
      class="cm-form-modal"
    >
      <NForm :model="form">
